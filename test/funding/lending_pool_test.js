...
contract('LendingPool', accounts => {
    ...
    it('can not borrow if market closed', async () => {
        await hydro.closeOrReopenMarket(0, true, { from: accounts[0] });
        await assert.rejects(borrow(0, USDAddr, toWei('100'), { from: u2 }), /MARKET_CLOSE/);
        await hydro.closeOrReopenMarket(0, false, { from: accounts[0] });
